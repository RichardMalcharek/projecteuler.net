...
#<p class="note"><b>NOTE:</b> Once the chain starts the terms are allowed 
# to go above one million.</p>


lim = 1000000
Starts = list(range(13,lim))
memo = {}
winner = [0,0]
...
for x in Starts:
    count = 0
    i = x
    print(f"candidate: {x} || corrent best - count: {winner[0]} Start:{winner[1]} || {lim-x} left")
    while i != 1:
        if i in memo:
            count += memo[i]
            break
